...
class InjectionModule(nn.Module):  
    def __init__(self, size):  
        ...
        self.size = size 
        self.linear = nn.Conv1d(in_channels=77, out_channels=size, kernel_size=3, stride=3, padding=1)
        self.norm = nn.LayerNorm([size**2, size])  
        self.relu = nn.ReLU()
...
class FlowNet(nn.Module):
    def __init__(self, ngf, norm=nn.BatchNorm2d, x=3):
        ...
        self.deconv1 = deconv(ngf * 2, ngf // 2, self.batchNorm)
        self.deconv0 = deconv(ngf // 2 + ngf // 4 + 2, ngf // 4, self.batchNorm)

        self.inter_conv1 = i_conv(ngf // 2 + ngf // 4 + 2, ngf // 2, self.batchNorm)
        self.inter_conv0 = i_conv(ngf // 4 + 2, ngf // 4, self.batchNorm)

        self.predict_flow2 = predict_flow(ngf * 2)
    ...
    def forward(self, x, cond):
        ...
        flow2 = self.predict_flow2(out_conv2)  # output: B*2*8*8

        flow2_up = self.upsampled_flow2_to_1(flow2)  # output: B*2*16*16
        out_deconv1 = self.deconv1(out_conv2) # output: B*2*16*16
        out_cond1 = self.injection1(cond)
        concat1 = th.cat((out_deconv1, flow2_up, out_cond1), 1)  # inter: B*x*16*16
        out_interconv1 = self.inter_conv1(concat1)
        flow1 = self.predict_flow1(out_interconv1)  # output: B*2*16*16

        flow1_up = self.upsampled_flow1_to_0(flow1)
        out_deconv0 = self.deconv0(concat1)
        concat0 = th.cat((out_deconv0, flow1_up), 1) # inter: B*x*32*32
        out_interconv0 = self.inter_conv0(concat0)
        flow0 = self.predict_flow0(out_interconv0)  # output: B*2*32*32
...
class WarpNet(nn.Module):
    ...
    def forward(self, images, flow, mode='bilinear'):
        return F.grid_sample(images, flow.transpose(1, 2).transpose(2, 3), mode=mode, align_corners=False)
